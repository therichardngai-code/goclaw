...
func init() {
	register(LocaleZH, map[string]string{
		...
		MsgSkillNudge70Pct:      "[System] 预算提示：已使用 70% 的迭代次数。请继续专注于用户的请求并提供完整输出。完成任务后，可以考虑将可重用的模式保存为技能。",
		MsgSkillNudge90Pct:      "[System] 预算提示：已使用 90% 的迭代次数。请优先完成并完整交付用户请求的结果。技能创建可以在任务完成后进行。",
	})
}
